...
async function loadServices(){
    ...
    services.forEach(x=>{
        ...
        e.onclick=()=>{document.querySelectorAll('#svc .option').forEach(el=>el.classList.remove('selected'));e.classList.add('selected');svc=x;};
        s.appendChild(e);
    });
}
...
async function loadMasters(){
    ...
    masters.forEach(x=>{
        ...
        e.onclick=()=>{document.querySelectorAll('#mst .option').forEach(el=>el.classList.remove('selected'));e.classList.add('selected');mst=x;};
        m.appendChild(e);
    });
}

function gd(){
    const g=document.getElementById('dt');g.innerHTML='';
    const t=new Date();
    for(let i=0;i<7;i++){
        ...
        b.onclick=()=>{document.querySelectorAll('#dt div').forEach(e=>e.classList.remove('selected'));b.classList.add('selected');date=ds;};
        g.appendChild(b);
    }
}

async function gt(){
    const g=document.getElementById('tm');g.innerHTML='';
    try{
        const r=await fetch(`/api/booked-slots?date=${date}&master_id=${mst.id}`);
        if(!r.ok)throw new Error('Error');
        const bk=(await r.json()).map(x=>x.time);
        for(let h=10;h<21;h++)for(let m=0;m<60;m+=30){
            const t=`${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`;
            const b=document.createElement('div');
            if(bk.includes(t)){b.className='booked';b.textContent=t;}
            else{b.textContent=t;b.onclick=()=>{document.querySelectorAll('#tm div').forEach(e=>e.classList.remove('selected'));b.classList.add('selected');time=t};}
            g.appendChild(b);
        }
    }catch(e){tg.showAlert('Ошибка загрузки времени')}
}
...
function sh(n){
    ...
    if(n===3)gd();if(n===4)gt();
    if(n===5){
        document.getElementById('sm_svc').textContent=svc.name;
        document.getElementById('sm_mst').textContent=mst.name;
        document.getElementById('sm_dt').textContent=date;
        document.getElementById('sm_tm').textContent=time;
        document.getElementById('sm_pr').textContent=svc.price+'₽';
    }
}
