...
class Settings(BaseSettings):
    ...
    @classmethod
    def parse_weekend_days(cls, value):
        if isinstance(value, str):
            return [int(x.strip()) for x in value.split(",") if x.strip()]
        if isinstance(value, list):
            return value
...
MASTERS = [
    {"id":1,"name":"Виктор","photo_url":None,"rating":4.9,"experience_years":8,"telegram_id":None,"max_bookings_per_day":15},
    {"id":2,"name":"Алексей","photo_url":None,"rating":4.8,"experience_years":5,"telegram_id":None,"max_bookings_per_day":15},
    {"id":3,"name":"Максим","photo_url":None,"rating":4.7,"experience_years":3,"telegram_id":None,"max_bookings_per_day":15},
]
SERVICES = [
...
app = FastAPI(title="Barbershop API", version="6.0.0", lifespan=lifespan)
...
class NotificationService:
    ...
    async def notify_manual_booking(cls, booking, client):
        try:
            bot = await cls.get_bot()
            text = f"📞 <b>Вас записали по звонку!</b>\n\nУслуга: {booking.service.name}\nМастер: {booking.master.name}\nДата: {booking.date}\nВремя: {booking.time}\nЦена: {booking.service.price}₽\n\n📍 ул. Чернышевского, 52Б\n\n<i>Для управления записью перейдите в бота:</i>"
            await bot.send_message(client.telegram_id, text)
            await bot.session.close()
            logger.info(f"Уведомление о ручной записи отправлено клиенту {client.telegram_id}")
        except Exception as e:
...
class BookingService:
    ...
    async def create_booking(self, telegram_id: int, chat_id: int, username: str | None, first_name: str | None, last_name: str | None, service_id: int, master_id: int, booking_date: str, booking_time: str):
        ...
        if day_count >= (master.max_bookings_per_day or settings.DEFAULT_MAX_BOOKINGS_PER_DAY):
            raise ValueError(f"У мастера лимит записей на этот день: {master.max_bookings_per_day}")
    ...
    async def create_manual_booking(self, client_name: str, phone: str | None, service_id: int, master_id: int, booking_date: str, booking_time: str, admin_id: int):
        ...
        booking = Booking(client_id=client.id, master_id=master.id, service_id=service.id, date=booking_date, time=booking_time, is_manual=True, manual_client_name=client_name, manual_phone=phone)
        await self.booking_repo.create(booking)
        await self.client_repo.increment_visits(client.id)
        await self.session.commit()
        await self.session.refresh(booking)
...
async def create_manual_booking(data: ManualBookingSchema, session: AsyncSession = Depends(get_session)):
    ...
    try:
        ...
        await audit.log(data.admin_telegram_id, "manual_booking", f"client={data.client_name} phone={data.phone}")
        await session.commit()
        return result
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def create_service(data: ServiceCreateSchema, session: AsyncSession = Depends(get_session)):
    ...
    audit = AuditRepository(session)
    await audit.log(data.admin_telegram_id, "create_service", f"name={data.name} price={data.price}")
    await session.commit()
...
async def toggle_service(service_id: int, data: ServiceToggleSchema, session: AsyncSession = Depends(get_session)):
    ...
    audit = AuditRepository(session)
    await audit.log(data.admin_telegram_id, "toggle_service", f"service_id={service_id} is_active={service.is_active}")
    await session.commit()
...
async def get_all_masters(admin_telegram_id: int, session: AsyncSession = Depends(get_session)):
    ...
    return [{"id": m.id, "name": m.name, "photo": m.photo_url, "rating": m.rating, "experience": m.experience_years, "telegram_id": m.telegram_id, "max_bookings": m.max_bookings_per_day, "is_active": m.is_active} for m in masters]
...
async def create_master(data: MasterCreateSchema, session: AsyncSession = Depends(get_session)):
    if not is_admin(data.admin_telegram_id):
        raise HTTPException(status_code=403, detail="Нет доступа")
    repo = MasterRepository(session)
    master = Master(name=data.name, photo_url=data.photo_url, experience_years=data.experience_years, telegram_id=data.telegram_id, max_bookings_per_day=data.max_bookings_per_day)
...
async def toggle_master(master_id: int, data: MasterToggleSchema, session: AsyncSession = Depends(get_session)):
    ...
    audit = AuditRepository(session)
    await audit.log(data.admin_telegram_id, "toggle_master", f"master_id={master_id} is_active={master.is_active}")
    await session.commit()
...
async def send_broadcast(data: BroadcastSchema, session: AsyncSession = Depends(get_session)):
    ...
    audit = AuditRepository(session)
    await audit.log(data.admin_telegram_id, "broadcast", f"sent={result['sent']} failed={result['failed']}")
    await session.commit()
    return result
EOF
...
let state = { screen: 'menu', svc: null, mst: null, date: null, time: null, services: [], masters: [], bookings: [], pastBookings: [], myReviews: [], profile: null, stats: null, todayBookings: [], allServices: [], allMasters: [], isSubmitting: false, todayFilterMaster: null, selectedPhotoFile: null, selectedPhotoPath: null, broadcastPhotoFile: null, manualSvc: null, manualMst: null, manualDate: null, manualTime: null };
...
function rn(screen) {
    ...
    else if (screen === 'admin_manual_booking') renderAdminManualBooking(app);
    else if (screen === 'manual_service') renderManualService(app);
    else if (screen === 'manual_master') renderManualMaster(app);
    else if (screen === 'manual_date') renderManualDate(app);
    else if (screen === 'manual_time') renderManualTime(app);
    else if (screen === 'manual_confirm') renderManualConfirm(app);
}
...
function renderBookingDate(app) {
    ...
    for (let i = 0; i < 14; i++) {
        ...
        if (WEEKEND_DAYS.includes(dow)) { b.className = 'weekend'; b.textContent += ' (вых)'; }
...
async function cf() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            const p = await api(`/api/profile?telegram_id=${user.id}`);
            if (p.exists) { state.profile = p; state.bookings = p.bookings || []; state.pastBookings = p.past_bookings_for_review || []; state.myReviews = p.my_reviews || []; }
            if (isAdmin) { state.stats = await api(`/api/admin/stats?admin_telegram_id=${user.id}`); state.todayBookings = await api(`/api/admin/today-bookings?admin_telegram_id=${user.id}`); }
            rn('my_bookings');
...
function renderAdminManualBooking(app) {
    app.innerHTML = '<h2>📞 Запись по звонку</h2><div class="form-group"><label>Имя клиента</label><input id="mclient" placeholder="Иван"></div><div class="form-group"><label>Телефон (необязательно)</label><input id="mphone" placeholder="+79001234567"></div><button class="btn-manual" onclick="rn(\'manual_service\')">Далее: выбор услуги</button><div class="btn-group"><button class="btn-back" onclick="rn(\'menu\')">← Назад</button></div>';
}
...
function renderManualService(app) {
    ...
    state.services.forEach(x => {
        ...
        e.className = 'option';
        e.innerHTML = `<div class="info"><b>${x.name}</b><span>${x.duration} мин</span></div><strong style="color:#c9a96e">${x.price}₽</strong>`;
        e.onclick = () => { state.manualSvc = x; rn('manual_master'); };
        c.appendChild(e);
    });
}
...
function renderManualMaster(app) {
    ...
    state.masters.forEach(x => {
        ...
        e.className = 'option';
        e.innerHTML = `<img src="${x.photo || ''}" onerror="this.style.display=\'none\'"><div class="info"><b>${x.name}</b><span>⭐${x.rating} | Опыт ${x.experience} лет</span></div>`;
        e.onclick = () => { state.manualMst = x; rn('manual_date'); };
        c.appendChild(e);
    });
}
...
function renderManualDate(app) {
    ...
    const g = document.getElementById('mdt');
    const t = new Date();
    for (let i = 0; i < 14; i++) {
        const d = new Date(t);
        d.setDate(t.getDate() + i);
...
async function renderManualTime(app) {
    ...
    for (let h = 10; h < 21; h++) {
        for (let m = 0; m < 60; m += 30) {
            ...
            const b = document.createElement('div');
            if (bt.includes(tm)) { b.className = 'booked'; b.textContent = tm; }
            else if (state.manualDate === today && (h < curH || (h === curH && m <= curM))) { b.className = 'booked'; b.textContent = tm; }
...
function renderManualConfirm(app) {
    app.innerHTML = '<h2>Подтверждение</h2><div class="summary"><div class="summary-item"><span>Клиент</span><strong id="mc_name"></strong></div><div class="summary-item"><span>Телефон</span><strong id="mc_phone"></strong></div><div class="summary-item"><span>Услуга</span><strong id="mc_svc"></strong></div><div class="summary-item"><span>Мастер</span><strong id="mc_mst"></strong></div><div class="summary-item"><span>Дата</span><strong id="mc_dt"></strong></div><div class="summary-item"><span>Время</span><strong id="mc_tm"></strong></div><div class="summary-item total"><span>Цена</span><strong id="mc_pr"></strong></div></div><div class="btn-group"><button class="btn-back" onclick="rn(\'manual_time\')">← Назад</button><button class="btn-confirm" onclick="manualCf()">Подтвердить</button></div>';
    document.getElementById('mc_name').textContent = document.getElementById('mclient')?.value || '—';
    document.getElementById('mc_phone').textContent = document.getElementById('mphone')?.value || '—';
    document.getElementById('mc_svc').textContent = state.manualSvc.name;
    document.getElementById('mc_mst').textContent = state.manualMst.name;
    document.getElementById('mc_dt').textContent = state.manualDate;
    document.getElementById('mc_tm').textContent = state.manualTime;
    document.getElementById('mc_pr').textContent = state.manualSvc.price + '₽';
}

async function manualCf() {
    if (state.isSubmitting) return;
    state.isSubmitting = true;
    const clientName = document.getElementById('mclient').value;
    const phone = document.getElementById('mphone').value || null;
    if (!clientName) { tg.showAlert('Введите имя клиента'); state.isSubmitting = false; return; }
    const payload = { admin_telegram_id: user.id, client_name: clientName, phone, service_id: state.manualSvc.id, master_id: state.manualMst.id, date: state.manualDate, time: state.manualTime };
    try {
...
async function loadTodayFiltered() {
    ...
    state.todayBookings.forEach(b => {
        ...
        const btn = document.createElement('button');
        btn.className = 'btn-cancel'; btn.textContent = '❌ Отменить'; btn.style.marginTop = '8px'; btn.style.width = '100%';
        btn.onclick = async () => {
...
function showMasterForm(editData = null) {
    const app = document.getElementById('app');
    state.selectedPhotoFile = null; state.selectedPhotoPath = editData?.photo || null;
...
echo ""
echo "=============================================="
echo "  ВСЁ! setup.sh готов. v6.0"
echo "  Все 16 пунктов + загрузка фото + ручная запись."
