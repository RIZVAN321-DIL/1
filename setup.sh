...
app = FastAPI(title="Barbershop API", version="5.0.0", lifespan=lifespan)
...
class MasterRepository:
    ...
    async def add_day_off(self, master_id: int, date: str, reason: str | None = None) -> MasterDayOff:
        ...
        await self.session.flush()
        return day_off
    async def get_days_off(self, master_id: int) -> list[MasterDayOff]:
        result = await self.session.execute(select(MasterDayOff).where(MasterDayOff.master_id == master_id).order_by(MasterDayOff.date))
        return list(result.scalars().all())
...
class BroadcastService:
    ...
    async def send_broadcast(text: str, session, photo_path: str | None = None):
        ...
        photo_bytes = None
        if photo_path:
            full_path = os.path.join(photo_path)
...
echo ""
echo "=============================================="
echo "  ВСЁ! setup.sh готов. v5.0"
echo "  Все 16 пунктов + загрузка фото."
